...
function BimSurfer() {
			...
			if (othis.viewport.mouse.pickRecord != null) {
				...
				if ($('#' + RegExp.escape(picknode.get("id"))).text().match(/^ dp_/)) {
					if (othis.propertyValues.selectedObj == ($('#' + RegExp.escape(picknode.get("id"))).text())) {
		    			...
		    			//GWT: 
						window.callbackAddDpWidget(othis.propertyValues.selectedObj,coords[0], coords[1]);
					}
					othis.viewport.mouse.leftDown = false;
					//GWT: 
					window.callbackClickEventDatapointMove(othis.propertyValues.selectedObj);
		...
		if (event.which === 1 && othis.viewport.mouse.leftDragDistance < othis.constants.mouse.pickDragThreshold
			...
			if (othis.viewport.mouse.pickRecord != null) {
				...
				if (othis.propertyValues.selectedObj != 'emtpy Selection') {
					//GWT: 
					window.callbackClickEventZoneMarked(othis.propertyValues.selectedObj);
				}
			} else {
	...
	this.mouseWheel = function(event) {
		...
		//GWT: 
		window.callbackZoomLevelAbsolute(othis.propertyValues.oldZoom);
	...
	window.setView = function(view) {
		...
		if (othis.scene != null) {
			...
			case (0): // reset
				...
				//GWT: 
				window.callbackZoomLevelAbsolute(15);
				break;
			case (1): // front
				othis.resetView();
				...
				//GWT: 
				window.callbackZoomLevelAbsolute(10);
				break;
			case (2): // side
				othis.resetView();
				lookAtNode = othis.scene.findNode('main-lookAt');
	...
	this.setExposeSlider = function(id) {
		...
		if (translateNode != null) {
			...
			//GWT: 
			window.callbackExposeLevel(othis.propertyValues.scalefactor*10*Math.abs(translateNode.get('y')));
		} else {
			($('#expose')).slider("value", 0);
			//GWT: 
			window.callbackExposeLevel(0);
		}
	};
	...
	/**
	  ...
	  this.setTransparentSlider = function (id) {
		  ...
		  if(transparentNode != null) {
			  //GWT: 
			  window.callbackTransparentLevel(100 - transparentNode.get('alpha')*100);
			  ($('#transparent')).slider("value" , 100 - transparentNode.get('alpha')*100);
		  }
		  else {
			  if(wallId != null) {
				  //GWT: 
				  window.callbackTransparentLevel(100);
...
//					node.insert('node', othis.constants.highlightMaterial);
	...
	this.controlsTreeScrollToSelected = function() {
		//MOST GWT Version
		var list = $('#controlsAccordionObjects');
	...
	window.fileImport = function(file) {
		console.log("Here");
		console.log(file);
		
//	    $.getJSON("ifc/AC11-FZK-Haus.json", function(json) {
...
//	    }); 
	...
	//initial load of a model
	window.callbackImportObject();
